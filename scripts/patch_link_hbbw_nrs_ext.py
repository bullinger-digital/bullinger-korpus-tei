...
class HBBWLinker:
    ...
    def tag_letter(self, path):
        ...
        m = re.match(r'(.*<ref target="[^"]*">[^<]*</ref>[,\df\-]*;\s*)(Nr\. (\d+)[a-z]?).*', s, flags=re.S)
        while m:
            if m.group(2) in self.map:
                s = re.sub(
                    re.escape(m.group(0)+m.group(1)),
                    m.group(0)+'<ref target="file'+self.map[m.group(2)]+'">'+m.group(1)+'</ref>',
                    s, flags=re.S
                )
            m = re.match(r'(.*<ref target="[^"]*">[^<]*</ref>[,\df\-]*;\s*)(Nr\. (\d+)[a-z]?).*', s, flags=re.S)
