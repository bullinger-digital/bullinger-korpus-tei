...
data = {}
with open("scripts/src/page_number_letters.csv") as fi:
    for line in fi:
        t = line.strip('\n').split(",\t")
...
for f in os.listdir(ROOT):
    if f.endswith('.xml'):
        ...
        m = re.match('.*<bibl source="b(\d+)" n="([^"]*)" type="transcription"/>', s, flags=re.S)
        if m:
            f_id = f.strip('.xml')
            fk_bib = m.group(1)
            num = m.group(2)
                ...
                for t in data[f_id]:
                    if fk_bib == t["fk_bib"]:
                        if num != t["num"] or (not num and t["num"]):
                            s = re.sub(
                                r'(<bibl source="b\d+" n=)"[^"]*"( type="transcription"/>)',
